(ns school.operation
  "OperationActor -- one school operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (SchoolOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Curriculum Safeguarding Governor (:govern) and
  ...
  One graph run = one school operation (intake -> advise -> govern ->
  ...
  decision to a human operator (the licensed educator). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:promotion/finalize`/`:safeguarding/finalize` ALWAYS reach this
  node when the governor is clean -- see `school.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [school.schoolopsllm :as schoolopsllm]
...
(defn build
  ...
  `school.store/Store`).
  opts:
    :advisor      -- a `school.schoolopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (schoolopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SchoolOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (schoolopsllm/-advise advisor store request)]
            {:proposal p :audit [(schoolopsllm/trace request p)]})))

      ;; Curriculum Safeguarding Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
