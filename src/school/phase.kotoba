(ns school.phase
  "Phase 0->3 staged rollout -- the school analog of `cloud-itonami-
  isic-6512`'s `casualty.phase`.
    ...
    Phase 3  supervised auto  -- governor-clean, high-confidence
                                 ...
                                 yet) may auto-commit. `:promotion/
                                 finalize`/`:safeguarding/finalize`
                                 NEVER auto-commit, at any phase.

  `:promotion/finalize`/`:safeguarding/finalize` are deliberately
  ...
  record are the two real-world legal acts this actor performs; both
  are always a human educator's call. `school.governor`'s `:actuation/
  finalize-promotion`/`:actuation/finalize-safeguarding-record`
  high-stakes gate enforces the same invariant independently -- two
  layers, not one, agree on this. `:background-check/screen` is
  ...
  member (`:student/intake`) -- this domain has no separate no-
  capital-risk 'file' lifecycle distinct from the student record
  itself.")

(def read-ops  #{})
(def write-ops #{:student/intake :jurisdiction/assess :background-check/screen
                 :promotion/finalize :safeguarding/finalize})

;; NOTE the invariant: `:promotion/finalize`/`:safeguarding/finalize`
...
(def phases
  ...
  {0 {:label "read-only"       :writes #{}                                                          :auto #{}}
   ...
   2 {:label "assisted-assess" :writes #{:student/intake :jurisdiction/assess :background-check/screen} :auto #{}}
...
(defn gate
  ...
  - `:promotion/finalize`/`:safeguarding/finalize` are never auto-
